...
to the read-time accessors in :mod:`cancerdata.expression`. Each shipped shard
...
Each shard's :class:`~cancerdata.expression.ShardDataset` names its core via
``build_attr``, so :mod:`cancerdata.expression` can regenerate a missing cohort
